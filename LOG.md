...
- **排障实录（chrome-devtools 真机）**：确认真机 iframe 已跑 dd6aa12（speakerKey 剥全部符号/白名单 .some 双向/roster 并集三特征齐），状态party=[🔮埃利奥特,🛡️弗兰克]、whitelist=[塔尼亚婆婆,🛡️弗兰克] 正确；已在 live iframe 注入 `[SB-TRACE]` 追踪（包 callShopLLM/requestShopLlm/applyLLMResult，逐行打印 narratorPass/activeBlocked/allowed 判定），等用户复测一轮拿证据。
