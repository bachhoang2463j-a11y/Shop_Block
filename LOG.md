...
- **旁白插话语义调整（用户拍板）**：OFF=身份句"请你扮演店主"，未点名时全部店主说，**点名队友时该队友仍必须插话一条**（触发器写"必须各输出一条对应队友的插话回应"；落地层新增 `options.requiredReplies`——`handleSendMessage` 传入本轮点名名单，旁白过滤放行被点名队友、模型自行扩写的其他队友台词照旧丢弃）；ON=身份句"请你扮演店主和1~2个随行队友"，每轮必带 1~2 条队友插话（在场队友不足时有多少说多少，未点名由插话队友任选），点名时插话必含点名对象（提示词强制，点名漏答仍走语义重试）。`mentionedTeammates` 移除旁白早退，点名检测在 OFF 下同样生效；旁白 OFF 的 system 名单不再收窄为仅店主，改为名单行注明"未点名时只有店主说话"。
- **真机拦截复盘与匹配放宽（用户反馈：`🛡️弗兰克` 台词被前端吃掉）**：真机 MMS 名册角色名带 emoji 前缀（🔮埃利奥特/🛡️弗兰克）而块内「在场成员」无前缀，上一版"块内∩底栏"交集口径在某种名单状态下把变体漏出白名单。放宽三处：①`speakerKey` 改为剥除**全部**非中文/字母数字字符（原 plainName 只剥前缀，防后缀/间隔号变体）；②`buildSpeakerRoster` 改为底栏∪块内**并集**按键去重（底栏显示名优先），不再做交集过滤；③`applyLLMResult` 白名单判定改为归一键相等或**双向包含**即放行（与 `findDialogueAnchor` 同口径），操控者变体仍双向排除（AI 永不代言玩家）。出场人设匹配保持底栏优先窄口径不变。仍会被拦的只剩完全编造的名单外名字。
- **排障实录（chrome-devtools 真机）**：确认真机 iframe 已跑 dd6aa12（speakerKey 剥全部符号/白名单 .some 双向/roster 并集三特征齐），状态party=[🔮埃利奥特,🛡️弗兰克]、whitelist=[塔尼亚婆婆,🛡️弗兰克] 正确；注入 `[SB-TRACE]` 追踪后用户复测三轮——**匹配层完全正常**：第1轮台词点名"弗兰克"→ `🛡️弗兰克` 归一放行入库；第2/3轮未点名被丢。被丢根因**不是匹配**而是 `narratorPass=false`：`handleSendMessage` 里 M5 时代的遗留代码在调 applyLLMResult 前**强制** `playerState.narratorEnabled=false`（注释"applyLLMResult 自己渲染对话行"，git 考古确认 M5 时 applyLLMResult 根本不读旁白状态、其原防目标已不存在），M15 加入旁白过滤后该强制覆盖使 **chat 场景永远按旁白关闭过滤**（enter/buy/sale 不受影响，故只有 chat 队友失声）——用户的旁白开关实际是开的。修复：删除强制覆盖与恢复对，chat 回归真实开关语义（开=队友台词过白名单即可显示；关=未点名丢/点名放行）。热补中和层已在 live iframe 验证等待用户复测，产物已重建待重导入。
- **立绘残留 demo 默认（用户反馈：刷新/切正则后偶发）**：chrome-devtools 实测 img.src=`Blacksmith_Machinist.png`（demo 铁匠）而 `shopState.portrait='占卜商妇'`——根因为 `loadKeeperPortrait` 每次调用持有**独立的** settled/timer/probe：boot 先探测 demo 铁匠图（index.html:6608），块载入后商店探测快速上屏，随后旧探测 onload/超时**晚到覆盖**（最后完成者赢）。修复：`keeperPortraitSeq` 代号守卫，新请求接管后旧探测/旧超时一律作废（顺带修掉旧超时晚到把非占卜店错误回退成占卜图的同类隐患）。已真机热修验证（img 切回 shop.png, 811px 正常），产物重建待重导入。
